...
export default class User extends Vue {
    ...
    login() {
        this.axios.post("http://localhost:3000/api/v1/users.json", this.query, {withCredentials: true}).then((response)=>{
            this.axios.post("http://localhost:3000/api/v1/session", this.query.user, {withCredentials: true}).then(()=>{
                this.$router.push("/")
            })
        }).catch((reason)=>{
